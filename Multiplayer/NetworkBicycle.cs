...
public class NetworkBicycle : NetworkBehaviour {
  ...
  //  private Sprint sprint;
    ...
    private void Start()
    {
        ...
        // Instanitating in the network is controlled through connection establishement. There the player is set to new position
        if (IsNetworking())
        {
    ...
    private void NetworkOnClientDisconnected(ulong obj)
    {
        if (IsServer)
        {
         ...
         //   NononLogger.Instance.LogDebug($"{GameObject.Find("ServerPlayerData").GetComponent<ServerPlayerData>().GetPlayerName(obj)} connected...");
        }
    }
    public override void OnNetworkSpawn()
    {
        ...
        ForPlayerName(NetworkManager.Singleton.LocalClientId);
    }
    ...
    private void ForPlayerName(ulong clientId)
    {
        if (IsServer)
        {
            ...
            //NononLogger.Instance.LogDebug($"{GameObject.Find("ServerPlayerData").GetComponent<ServerPlayerData>().GetPlayerName(clientId)} connected...");
